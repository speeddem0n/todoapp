...
type Authorization interface { // Интерфейс для авторизации
	CreateUser(user models.User) (int, error)                // Создает нового пользователя и возвращает его Id
...
type TodoList interface { // Интерфейс для работы со списками
	Create(userId int, list models.CreateListInput) (int, error)   // Метод для создания списка возвращает id созданного списка и ошибку
...
type TodoItem interface { // Интерфейс для работы с элеметоми списков
	Create(userId, listId int, item models.CreateTodoItemInp) (int, error) // Метод для создания "todo" элемента возвращает id созданного элемента и ошибку
